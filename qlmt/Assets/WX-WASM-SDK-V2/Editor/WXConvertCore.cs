...
namespace WeChatWASM
{
    public class WXConvertCore
    {
        ...
        private static void convertDataPackage(bool brotliError)
        {
            ...
            if (brotliError)
            {
                // brotli失败后，因为无法知道wasmcode大小，无法准确判断小游戏总包体大小。
                if (config.ProjectConf.assetLoadType == 1)
                {
                    UnityEngine.Debug.LogWarning("brotli失败，无法检测文件大小，保持小游戏包内加载方式不变");
                }
            ...
            if (config.ProjectConf.assetLoadType == 1)
            {
                long packageLimitBytes = (30 - 1) * 1024 * 1024;
                // 计算wasm包大小
                var brcodePath = Path.Combine(minigameDirPath, "wasmcode", codeMd5 + ".webgl.wasm.code.unityweb.wasm.br");
                var brcodeSize = File.Exists(brcodePath) ? new FileInfo(brcodePath).Length : 0;
                // 计算首资源包大小
                var tempDataInfo = new FileInfo(tempDataPath);
                var tempFileSize = tempDataInfo.Length;
                var totalPackageSize = brcodeSize + tempFileSize;
                // 胶水层及sdk可能占一定大小，粗略按照1M来算，则剩余29M
                if (totalPackageSize > packageLimitBytes)
                {
                    var overflowSize = totalPackageSize - packageLimitBytes;
                    Debug.LogWarning(
                        "资源文件过大，继续使用小游戏包内加载。" +
                        $"wasm包大小: {FormatSize(brcodeSize)}，" +
                        $"首资源包大小: {FormatSize(tempFileSize)}，" +
                        $"总大小: {FormatSize(totalPackageSize)}，" +
                        $"阈值: {FormatSize(packageLimitBytes)}，" +
                        $"超出: {FormatSize(overflowSize)}");
                }

                // 小游戏分包加载时，始终将数据包copy到小游戏目录
        ...
        {
            return $"{sizeBytes / (1024f * 1024f):F2}MB ({sizeBytes} bytes)";
        }
